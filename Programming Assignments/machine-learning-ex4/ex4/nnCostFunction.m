...
for i=1:m
    ...
    for j=1:num_labels
       term1=(-1)*(y_vec(j));
       term1=term1*(log(h_x(j)));
       term2=(-1)*(1-y_vec(j));
       term2=term2*(log(1-(h_x(j))));
...
reg_term1=sum(sum(Theta1(:,2:dim1).^ 2));
reg_term2=sum(sum(Theta2(:,2:dim2).^ 2));
regularizer=(lambda/(2*m))*(reg_term1+reg_term2);
